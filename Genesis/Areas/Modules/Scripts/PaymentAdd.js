...
var viewModel = function () {
    ...
    _self.supplierDocuments = ko.observableArray();
    ...
    _self.paymentsChanged = function (existingPayments, event) {
        ...
        if ($("#ddPayments").val() != "") {
            //$("#ddsupplierDocuments").prop('disabled', 'disabled');
    ...
    //General
    _self.loadsupplierDocuments = function () {


        $("#ddSupplierDocuments").prop('disabled', 'disabled');
            var dataUrl = $("#hdnGetSupplierDocumentsUrl").attr("data-url");
            $.ajax({
                url: dataUrl,
                type: 'POST',
                dataType: 'json',
                async: false,
                data: { id: $("#ddSupplier").val() },
                success: function (d) {
                    _self.supplierDocuments(d);
                    $("#ddSupplierDocuments").prop('disabled', false);
                },
                error: function (e) {
                    alert("Ajax error[loadsupplierDocuments]: " + e);
                }
            });
    ...
    //General
    _self.removePurchaseDocument = function () {
    ...
    _self.clearDocumentAdd = function () {
        ...
        _self.amountToPay('');
        $('#ddSupplierDocuments').val('');

    };

    //General
    _self.documentChanged = function (supplierDocuments, event) {
            ...
            if ($('#txtAmountToPay').val() != "" && Number($('#txtAmountToPay').val()) > 0) {
                if (Number($('#txtAmountToPay').val()) <= paymentTotal && (Number(_self.GrandTotal()) + Number($('#txtAmountToPay').val())) <= paymentTotal) {
                    ...
                    if (Number($('#txtAmountToPay').val()) <= Number(_self.totalRemaining())) {
                        ...
                        var documentNumClean = $('#ddSupplierDocuments option:selected').text();
                        ...
                        var newDocument = {
                            ...
                            documentId:  $('#ddSupplierDocuments option:selected').val()
                        };
    ...
    _self.addPurchasePayment = function () {
                            ...
                            if ($("#txtIssuingBankNew").val() != "") {
                                if (_self.payDocuments().length > 0) {
                                    ...
                                    $.ajax({
                                        ...
                                        error: function (e) {
                                            console.log("Ajax error[addPurchasePayment]: " + JSON.stringify(e));
                                    }
                                    });
                                } else {
    ...
    var dataUrl = $("#hdnGetSuppliersUrl").attr("data-url");
